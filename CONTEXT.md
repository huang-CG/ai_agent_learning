...
> 极简进度文档，供每日对话快速恢复上下文  
> 最后更新：2026-08-26 | 状态：**D43 ✅（CLI 完整）→ 下次 D44（Web UI）**
...
Day:    43 / 90 ✅
Phase:  3（进行中）
Next:   D44（智能文档助手 · Web UI）
```

### 复盘日程（Agent 维护 · 2026-08-26）

| 类型 | 周期 | 时长 | 期间 | 封顶 | 下次触发 | 状态 |
|------|------|------|------|------|----------|------|
| **周复盘** | 每 **7 个学习日** | 固定 2 天 | 暂停每日新课 | 最多 2 天 | D42 起计 **2/7** | ✅ 第 2 次完成（D35–D41） |
...
| 3 | D28–47 | 🔄 16/20 | RAG Agent + Web UI |
...
|-----|------|------|----------|------|------|-------|
| D43 | 2026-08-26 | 2.5h | CLI多轮✅；记名字✅；读memo有tool✅；空输入/q退出✅；345元音双指针✅；理论C/A | Tab补全多、自感拼凑；终端误粘贴当输入；再见曾印两遍 | 4 | 3 |
...
**D43**：交互 CLI + 多轮 messages 历史；空输入/quit；读文件错名能诚实说明；力扣 345 元音双指针

**薄弱点（持续）**：语法结构；力扣多指针下标；读官方示例易和第三方混淆；交互脚本建议外部 PowerShell 跑（Cursor 终端重绘 bug）；低代码平台（Dify）首次配置易绕；异或等位运算需再练；**OOP 继承 / 对象 vs 类** 需多对照例子；走读时易把**注释当实现**（如 SQL `startswith` 大小写）；工具选型口述偶混（读文件≠SQL）；**中型项目易靠 Tab 补全拼通，需加强「先口述再写 / 少补全」**
...
| 2026-08-26 | **D43 完成**，自评 4 / Agent 3；文档助手 CLI 完整；力扣 345；周复盘 **2/7**；确认少 Tab、先口述再写 |
| 2026-08-26 | **学习笔记排版**：进度表 → 周复盘摘要（新在上）→ 每日 Day（新在上）；新 Day 禁止插到周复盘上面 |
...
| `exercises/day43/doc_assistant_cli.py` | D43 智能文档助手交互 CLI |
| `exercises/day43/README.md` | D43 验收与时间盒 |
...
### 学习笔记排版（2026-08-26 学员确认）

`notes/学习笔记.md` 正文顺序固定为：

1. **学习进度表**（顶端）
2. **周复盘 / 阶段复盘摘要**（新在上；全文仍在 `notes/复盘归档.md`）
3. **每日 Day 笔记**（新在上：D42 → D41 → …）

**硬性**：新写的 Day 节只能插在「全部周复盘摘要」**之后**、旧 Day 节之前；**禁止**把 Day 插到周复盘区块上面。
