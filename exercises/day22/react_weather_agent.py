"""
Day 22 · 手写 ReAct Agent（二）· 真天气 API

在 D21 主循环基础上：
  - get_weather → wttr.in（同 D4）
  - calculator → 受限 eval
  - Observation = 程序真实执行结果
  - max_steps 防死循环
...
def get_weather(city: str) -> str:
    """调用 wttr.in，返回一行中文天气摘要（Observation 用）。"""
    city = city.strip()
    if not city:
        return "错误：城市名不能为空"
...
def run_tool(tool_name: str, tool_arg: str) -> str:
    """根据 Action 路由到真实函数，返回 Observation 字符串。"""
...
def run_react(question: str, max_steps: int = 5) -> str:
    """
    ReAct 主循环。
    Observation 来自 run_tool（真 API / 真计算）。
    """
    messages: list[dict] = [
...
def main() -> None:
    ...
    print("Day 22 · ReAct Agent（真天气）", flush=True)
    print(
        "输入 quit 退出。验收：北京天气 / 广州现在气温多少？\n",
        flush=True,
    )
